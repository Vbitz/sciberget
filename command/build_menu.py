...
def add_menu(installdir: Path, name: Text, category: Text) -> None:
    """Add a submenu to 'sci-ber-get' menu.
    ...
    menu_path = installdir/"sciberget-applications.menu"
    ...
    for menu_el in root.findall(".//Menu/Menu"):
        ...
        # menu_el = root.findall("./Menu/Menu")[0]
            ...
            dir_el.text = f'sciberget/apps/{file_path.name}'
...
def neurodesk_xml(xml: Path, newxml: Path) -> None:
    oldtag = '<Menu>'
    newtag = '<MergeFile>sciberget-applications.menu</MergeFile>'
    replace = True
...
def build_menu(installdir, deskenv, sh_prefix):
    ...
    copyfile_with_mode(Path('command/sciberget-applications.menu'), installdir/'sciberget-applications.menu')
    copyfile_with_mode(Path('command/fetch_and_run.sh'), installdir/'fetch_and_run.sh', mode=0o755)
    copyfile_with_mode(Path('command/fetch_containers.sh'), installdir/'fetch_containers.sh', mode=0o755)
    copyfile_with_mode(Path('command/configparser.sh'), installdir/'configparser.sh', mode=0o755)
    copyfile_with_mode(Path('config.ini'), installdir/'config.ini')
    copyfile_with_mode(Path('command/apps.json'), installdir/'apps.json')
    distutils.dir_util.copy_tree('command/transparent-singularity', str(installdir/'transparent-singularity'))

    if not climode:
        directories_path = installdir/"desktop-directories"
        icon_dir = installdir/"icons"
        write_directory_file("sci-ber-get", directories_path, icon_dir)
        write_directory_file("All Applications", directories_path, icon_dir)
        write_directory_file("Information Gathering", directories_path, icon_dir)
        write_directory_file("Vulnerability Analysis", directories_path, icon_dir)
        write_directory_file("Web Applications", directories_path, icon_dir)
        write_directory_file("Database Assessment", directories_path, icon_dir)
        write_directory_file("Password Attacks", directories_path, icon_dir)
        write_directory_file("Wireless", directories_path, icon_dir)
        write_directory_file("Reverse Engineering", directories_path, icon_dir)
        write_directory_file("Exploitation", directories_path, icon_dir)
        write_directory_file("Sniffing Spoofing", directories_path, icon_dir)
        write_directory_file("Post Exploitation", directories_path, icon_dir)
        write_directory_file("Forensics", directories_path, icon_dir)
        write_directory_file("Reporting", directories_path, icon_dir)
        write_directory_file("Crypto Stego", directories_path, icon_dir)
        write_directory_file("Hardware", directories_path, icon_dir)
        write_directory_file("Fuzzing", directories_path, icon_dir)
        write_directory_file("Other", directories_path, icon_dir)

    appsjson = Path('command/apps.json').resolve(strict=True)
    (installdir/'icons').mkdir(exist_ok=True)
    apps_from_json(climode, deskenv, installdir, appsjson, sh_prefix)

    # sci-ber-get help app
    help_app = ScibergetApp(
        deskenv=deskenv,
        installdir=installdir,
        name="Help",
        category="sci-ber-get")
    help_app.app_names()
    help_app.add_app_sh("firefox https://sciberget.example.org")
    if not climode:
        help_app.add_app_menu()

    # Update Neurocommand app
    update_app = ScibergetApp(
        deskenv=deskenv,
        installdir=installdir,
        name="Update",
        category="sci-ber-get")
    update_app.app_names()
    update_app.add_app_sh(f"cd {installdir}/sciberget-command; bash build.sh --update --runsudo; read -p \"Press enter to close this window ...\"")
    if not climode:
